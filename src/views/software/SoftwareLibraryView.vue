  ...
  <div class="shelf-page">
    ...
    <section class="shelf-card shelf-filter-card">
      ...
      <div v-show="filterOpen" class="shelf-filter-body">
        <!-- 软件筛选 -->
        <div class="shelf-filter-grid shelf-filter-grid--software">
      ...
      <div class="shelf-table-wrap">
        ...
        <table class="shelf-table">
          <thead>
            <tr>
              <th>软件名称</th>
              <th class="shelf-th-sort" @click="toggleSort('score')">
                最新版本评分
                <svg class="shelf-sort-svg" viewBox="0 0 1024 1024" width="16" height="16">
                  <path d="M692.705882 572.235294l-180.705882 240.941177-180.705882-240.941177h361.411764z" :fill="sortField === 'score' && sortDir === 'desc' ? '#374151' : '#d1d5db'" />
                  <path d="M512 150.588235l180.705882 240.941177H331.294118L512 150.588235z" :fill="sortField === 'score' && sortDir === 'asc' ? '#374151' : '#d1d5db'" />
                </svg>
              </th>
              <th class="shelf-th-sort" @click="toggleSort('time')">
                版本发布时间
                <svg class="shelf-sort-svg" viewBox="0 0 1024 1024" width="16" height="16">
                  <path d="M692.705882 572.235294l-180.705882 240.941177-180.705882-240.941177h361.411764z" :fill="sortField === 'time' && sortDir === 'desc' ? '#374151' : '#d1d5db'" />
                  <path d="M512 150.588235l180.705882 240.941177H331.294118L512 150.588235z" :fill="sortField === 'time' && sortDir === 'asc' ? '#374151' : '#d1d5db'" />
                </svg>
              </th>
              <th>行业分类</th>
              <th class="shelf-th-sort" @click="toggleSort('vuln')">
                最新版本漏洞数
                <svg class="shelf-sort-svg" viewBox="0 0 1024 1024" width="16" height="16">
                  <path d="M692.705882 572.235294l-180.705882 240.941177-180.705882-240.941177h361.411764z" :fill="sortField === 'vuln' && sortDir === 'desc' ? '#374151' : '#d1d5db'" />
                  <path d="M512 150.588235l180.705882 240.941177H331.294118L512 150.588235z" :fill="sortField === 'vuln' && sortDir === 'asc' ? '#374151' : '#d1d5db'" />
                </svg>
              </th>
              <th>开发商</th>
              <th>编程语言</th>
      ...
      <footer class="shelf-footer">
        ...
        <div class="shelf-pager">
          ...
          <label class="shelf-page-size">
            ...
            <SearchSelect v-model="pageSize" class="shelf-page-select" number :options="PAGE_SIZE_OPTIONS" />
          </label>
        </div>
      </footer>
    </section>
...
const filteredList = computed(() => {
  let list = activeTab.value === 'software' ? filteredSw.value : filteredComp.value
  if (sortField.value === 'score') {
    list = [...list].sort((a, b) => sortDir.value === 'asc' ? (a.score || 0) - (b.score || 0) : (b.score || 0) - (a.score || 0))
  } else if (sortField.value === 'time') {
    list = [...list].sort((a, b) => sortDir.value === 'asc' ? (a.publishTime || '').localeCompare(b.publishTime || '') : (b.publishTime || '').localeCompare(a.publishTime || ''))
  } else if (sortField.value === 'vuln') {
    list = [...list].sort((a, b) => sortDir.value === 'asc' ? (a.vulnCount || 0) - (b.vulnCount || 0) : (b.vulnCount || 0) - (a.vulnCount || 0))
  }
  return list
})
...
.shelf-th-sort { cursor: pointer; user-select: none; white-space: nowrap; }
.shelf-sort-svg { vertical-align: middle; width: 18px; height: 18px; display: inline-block; }
